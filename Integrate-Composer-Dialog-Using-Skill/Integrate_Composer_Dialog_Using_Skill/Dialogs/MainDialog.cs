    ...
    public class MainDialog : ComponentDialog
    {
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                var skillResult = await localizedServices.LuisServices["Integrate_Composer_Dialog_Using_Skill"].RecognizeAsync<Integrate_Composer_Dialog_Using_SkillLuis>(innerDc.Context, cancellationToken);
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("Integrate_Composer_Dialog_Using_Skill", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<Integrate_Composer_Dialog_Using_SkillLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case Integrate_Composer_Dialog_Using_SkillLuis.Intent.Sample:
                            {
                                return await stepContext.BeginDialogAsync(_sampleDialog.Id, cancellationToken: cancellationToken);
                            }

                        case Integrate_Composer_Dialog_Using_SkillLuis.Intent.None:
                        default:
                            {
